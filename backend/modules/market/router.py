...
async def get_prices(
    ...
    prices = await service.get_prices(db, plant_id, region, days)
...
async def get_trend(
    ...
    data = await service.get_trend(db, plant_id, region)
...
async def estimate_revenue(
    ...
    data = await service.estimate_revenue(db, project_id, current_user.id, region)
    return success_response(data.model_dump())
