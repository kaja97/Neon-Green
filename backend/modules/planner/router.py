...
from dependencies import get_current_user, get_planner_service
...
async def get_today_activities(
    ...
    activities = await planner_service.get_today_activities(db, project_id, current_user.id)
...
async def list_activities(
    ...
    activities = await planner_service.list_activities(db, project_id, current_user.id)
    return success_response([schemas.ActivityResponse.model_validate(a).model_dump() for a in activities])
...
async def create_activity(
    ...
    activity = await planner_service.create_activity(db, project_id, current_user.id, data)
...
async def update_activity(
    ...
    activity = await planner_service.update_activity(db, activity_id, current_user.id, data)
    return success_response(schemas.ActivityResponse.model_validate(activity).model_dump())
...
async def delete_activity(
    ...
    await planner_service.delete_activity(db, activity_id, current_user.id)
    return None
...
async def mark_activity_complete(
    ...
    activity = await planner_service.mark_complete(db, activity_id, current_user.id, data)
    return success_response(schemas.ActivityResponse.model_validate(activity).model_dump())

@router.patch("/activities/{activity_id}/skip", status_code=200)
async def mark_activity_skip(
    activity_id: uuid.UUID,
    data: schemas.SkipRequest,
    current_user: Account = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    planner_service: PlannerService = Depends(get_planner_service)
):
    activity = await planner_service.mark_skip(db, activity_id, current_user.id, data)
    return success_response(schemas.ActivityResponse.model_validate(activity).model_dump())
...
async def reset_activity(
    ...
    activity = await planner_service.reset_activity(db, activity_id, current_user.id)
    return success_response(schemas.ActivityResponse.model_validate(activity).model_dump())
