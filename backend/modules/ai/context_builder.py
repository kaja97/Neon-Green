...
async def build_project_context(db: AsyncSession, project_id: uuid.UUID) -> str:
    ...
    try:
        ...
        context = await _agro_ctx(db, project_id, mode="full")
        if "error" in context:
        ...
        return await _fallback_context(db, project_id)


async def _fallback_context(db: AsyncSession, project_id: uuid.UUID) -> str:
