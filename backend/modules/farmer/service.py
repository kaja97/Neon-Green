...
async def _get_profile_or_raise(db: AsyncSession, account_id: uuid.UUID) -> FarmerProfile:
...
async def get_profile(db: AsyncSession, account_id: uuid.UUID) -> FarmerProfile:
    return await _get_profile_or_raise(db, account_id)


async def update_profile(
    db: AsyncSession, account_id: uuid.UUID, data: FarmerProfileUpdate
) -> FarmerProfile:
    profile = await _get_profile_or_raise(db, account_id)
...
async def create_location(
    db: AsyncSession, account_id: uuid.UUID, data: LocationCreate
) -> FarmerLocation:
    profile = await _get_profile_or_raise(db, account_id)
...
async def get_location(
    db: AsyncSession, account_id: uuid.UUID, location_id: uuid.UUID
) -> FarmerLocation:
    profile = await _get_profile_or_raise(db, account_id)
    result = await db.execute(
...
async def create_land_detail(
    db: AsyncSession, account_id: uuid.UUID, data: LandDetailCreate
) -> FarmerLandDetail:
    profile = await _get_profile_or_raise(db, account_id)
...
async def list_land_details(
    ...
    profile = await _get_profile_or_raise(db, account_id)
    result = await db.execute(
        select(FarmerLandDetail).where(FarmerLandDetail.farmer_id == profile.id)
    )
    return result.scalars().all()


async def get_land_detail(
    db: AsyncSession, account_id: uuid.UUID, land_id: uuid.UUID
) -> FarmerLandDetail:
    profile = await _get_profile_or_raise(db, account_id)
    result = await db.execute(
...
async def create_livestock(
    db: AsyncSession, account_id: uuid.UUID, data: LivestockCreate
) -> FarmerLivestock:
    profile = await _get_profile_or_raise(db, account_id)
...
async def list_livestock(
    ...
    profile = await _get_profile_or_raise(db, account_id)
    result = await db.execute(
        select(FarmerLivestock).where(FarmerLivestock.farmer_id == profile.id)
    )
    return result.scalars().all()


async def get_livestock(
    db: AsyncSession, account_id: uuid.UUID, livestock_id: uuid.UUID
) -> FarmerLivestock:
    profile = await _get_profile_or_raise(db, account_id)
    result = await db.execute(
